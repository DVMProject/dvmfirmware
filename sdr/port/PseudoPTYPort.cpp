...
PseudoPTYPort::PseudoPTYPort(const std::string& symlink, SERIAL_SPEEDS speed, bool assertRTS) : UARTPort(speed, assertRTS),
    m_symlink(symlink)
{
    /* stub */
}
...
bool PseudoPTYPort::open()
{
	...
	if (result < 0) {
		::LogError(LOG_DSP, "Cannot open the pseudo tty - errno : %d", errno);
		return false;
	}
	...
	if (ret != 0) {
		::LogError(LOG_DSP, "Cannot make symlink to %s with %s", slave, m_symlink.c_str());
		close();
		return false;
	}

	::LogMessage(LOG_DSP, "Made symbolic link from %s to %s", slave, m_symlink.c_str());
