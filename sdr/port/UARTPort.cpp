...
UARTPort::UARTPort(const std::string& device, UART_SERIAL_SPEED speed, bool assertRTS, bool rtsBoot) :
    m_isOpen(false),
    m_device(device),
    m_speed(speed),
...
int UARTPort::write(const uint8_t* buffer, uint32_t length)
{
    ...
    while (ptr < length) {
        ...
        if (n < 0) {
            if (errno != EAGAIN) {
                ::LogError("Error returned from write(), errno=%d (%s)", errno, strerror(errno));
                return -1;
            }
        }
...
UARTPort::UARTPort(UART_SERIAL_SPEED speed, bool assertRTS, bool rtsBoot) :
    m_isOpen(false),
    m_speed(speed),
    m_assertRTS(assertRTS),
    m_rtsBoot(rtsBoot),
...
bool UARTPort::setTermios()
{
    ...
    if (m_rtsBoot) {
        ::LogInfoEx("RTS/DTR boot flag enabled, forcing board reset");
        uint32_t y;
        if (::ioctl(m_fd, TIOCMGET, &y) < 0) {
            ::LogError("Cannot get the control attributes for %s", m_device.c_str());
            ::close(m_fd);
