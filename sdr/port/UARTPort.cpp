...
UARTPort::UARTPort(const std::string& device, SERIAL_SPEEDS speed, bool assertRTS) : 
    m_isOpen(false),
    m_device(device),
    m_speed(speed),
...
#endif
    if (m_fd < 0) {
        ::LogError(LOG_DSP, "Cannot open device - %s", m_device.c_str());
        return false;
    }

    if (::isatty(m_fd) == 0) {
        LogError(LOG_DSP, "%s is not a TTY device", m_device.c_str());
        ::close(m_fd);
        return false;
    }
...
int UARTPort::read(uint8_t* buffer, uint32_t length)
{
    ...
    while (offset < length) {
        ...
        if (n < 0) {
            ::LogError(LOG_DSP, "Error from select(), errno=%d", errno);
            return -1;
        }
        ...
        if (n > 0) {
            ...
            if (len < 0) {
                if (errno != EAGAIN) {
                    ::LogError(LOG_DSP, "Error from read(), errno=%d", errno);
                    return -1;
                }
            }
...
int UARTPort::write(const uint8_t* buffer, uint32_t length)
{
    ...
    while (ptr < length) {
        ...
        if (n < 0) {
            if (errno != EAGAIN) {
                ::LogError(LOG_DSP, "Error returned from write(), errno=%d", errno);
                return -1;
            }
        }
...
/// <param name="speed">Serial port speed.</param>
/// <param name="assertRTS"></param>
UARTPort::UARTPort(SERIAL_SPEEDS speed, bool assertRTS) :
    m_isOpen(false),
    m_speed(speed),
    m_assertRTS(assertRTS),
    m_fd(-1)
...
bool UARTPort::setTermios()
{
    ...
    if (::tcgetattr(m_fd, &termios) < 0) {
        ::LogError(LOG_DSP, "Cannot get the attributes for %s", m_device.c_str());
        ::close(m_fd);
        return false;
    }
    ...
    case SERIAL_460800:
        ...
        break;
    default:
        ::LogError(LOG_DSP, "Unsupported serial port speed - %u", m_speed);
        ::close(m_fd);
        return false;
    }

    if (::tcsetattr(m_fd, TCSANOW, &termios) < 0) {
        ::LogError(LOG_DSP, "Cannot set the attributes for %s", m_device.c_str());
        ::close(m_fd);
        return false;
    }
    ...
    if (m_assertRTS) {
        ...
        if (::ioctl(m_fd, TIOCMGET, &y) < 0) {
            ::LogError(LOG_DSP, "Cannot get the control attributes for %s", m_device.c_str());
            ::close(m_fd);
            return false;
        }
        ...
        if (::ioctl(m_fd, TIOCMSET, &y) < 0) {
            ::LogError(LOG_DSP, "Cannot set the control attributes for %s", m_device.c_str());
            ::close(m_fd);
            return false;
        }
    }
